...
{
    // TODO: this doesn't really belong here. This infra is more general.
    if (mGroupMap.find(val) != mGroupMap.end()) {
        val = mGroupMap.find(val)->second;
    } else {
        if (val->isForwardDefined()) {
            if (val->isResolved()) {
                val = val->forwardResolvesTo();
            }
        }
    }
