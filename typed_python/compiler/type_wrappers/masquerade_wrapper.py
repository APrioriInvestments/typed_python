...
class MasqueradeWrapper(Wrapper):
    ...
    def convert_to_type_with_target(self, context, instance, targetVal, conversionLevel, mayThrowOnFailure=False):
        ...
        if (
            targetVal.expr_type.typeRepresentation == self.typeRepresentation
        ) and conversionLevel.isImplicitContainersOrHigher():
            targetVal.convert_copy_initialize(instance.convert_masquerade_to_typed())
            return context.constant(True)

        if (
            conversionLevel == ConversionLevel.Signature
            and issubclass(self.interpreterTypeRepresentation, targetVal.expr_type.typeRepresentation)
        ):
            targetVal.convert_copy_initialize(instance.convert_masquerade_to_untyped())
            return context.constant(True)

        # Allow the typed form of the object to perform the conversion
        return self.convert_masquerade_to_typed(context, instance).convert_to_type_with_target(targetVal, conversionLevel)
    ...
    def convert_fastnext(self, context, instance):
        return self.convert_masquerade_to_typed(context, instance).convert_fastnext()

    def convert_attribute(self, context, instance, attribute):
        return self.convert_masquerade_to_typed(context, instance).convert_attribute(attribute)

    def convert_set_attribute(self, context, instance, attribute, value):
        return self.convert_masquerade_to_typed(context, instance).convert_set_attribute(attribute, value)

    def convert_delitem(self, context, instance, item):
        return self.convert_masquerade_to_typed(context, instance).convert_delitem(item)

    def convert_getitem(self, context, instance, item):
        return self.convert_masquerade_to_typed(context, instance).convert_getitem(item)

    def convert_getslice(self, context, instance, lower, upper, step):
        return self.convert_masquerade_to_typed(context, instance).convert_getslice(lower, upper, step)

    def convert_setitem(self, context, instance, index, value):
        return self.convert_masquerade_to_typed(context, instance).convert_setitem(index, value)

    def convert_call(self, context, instance, args, kwargs):
        return self.convert_masquerade_to_typed(context, instance).convert_call(args, kwargs)

    def convert_len(self, context, instance):
        return self.convert_masquerade_to_typed(context, instance).convert_len()

    def convert_hash(self, context, instance):
        return self.convert_masquerade_to_typed(context, instance).convert_hash()

    def convert_abs(self, context, instance):
        return self.convert_masquerade_to_typed(context, instance).convert_abs()

    def convert_builtin(self, f, context, instance, a1=None):
        return self.convert_masquerade_to_typed(context, instance).convert_builtin(f, a1)

    def convert_repr(self, context, instance):
        return self.convert_masquerade_to_typed(context, instance).convert_repr()

    def convert_unary_op(self, context, instance, op):
        return self.convert_masquerade_to_typed(context, instance).convert_unary_op(op)

    def _can_convert_to_type(self, otherType, conversionLevel):
        if otherType.typeRepresentation is self.typeRepresentation and conversionLevel.isImplicitContainersOrHigher():
            return True

        if otherType.typeRepresentation is self.interpreterTypeRepresentation:
            return True
    ...
    def convert_bin_op(self, context, l, op, r, inplace):
        return self.convert_masquerade_to_typed(context, l).convert_bin_op(op, r, inplace)

    def convert_bin_op_reverse(self, context, r, op, l, inplace):
        return self.convert_masquerade_to_typed(context, r).convert_bin_op_reverse(op, l, inplace)

    def convert_format(self, context, instance, formatSpecOrNone=None):
        return self.convert_masquerade_to_typed(context, instance).convert_format(formatSpecOrNone)

    def convert_type_call(self, context, typeInst, args, kwargs):
        assert False, "this should never be called"

    def convert_call_on_container_expression(self, context, inst, argExpr):
        return self.convert_masquerade_to_typed(context, inst).convert_call_on_container_expression(argExpr)

    def convert_type_call_on_container_expression(self, context, typeInst, argExpr):
        assert False, "this should never be called"

    def convert_method_call(self, context, instance, methodname, args, kwargs):
        return self.convert_masquerade_to_typed(context, instance).convert_method_call(methodname, args, kwargs)

    def convert_context_manager_enter(self, context, instance):
        return self.convert_masquerade_to_typed(context, instance).convert_context_manager_enter()

    def convert_context_manager_exit(self, context, instance, args):
        return self.convert_masquerade_to_typed(context, instance).convert_context_manager_exit(args)
