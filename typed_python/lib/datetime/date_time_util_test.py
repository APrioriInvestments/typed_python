...
def test_daterange():
    ...
    assert daterange(Date(2021, 12, 31), Date(2023, 1, 31), "months", stepSize=2) == ListOf(
        Date
    )([Date(y, m, d) for (y, m, d) in ymd])
    # leap year
    ymd = [(2020, 2, 29), (2020, 5, 29)]
    assert daterange(Date(2020, 2, 29), Date(2020, 7, 31), "months", stepSize=3) == ListOf(
        Date
    )([Date(y, m, d) for (y, m, d) in ymd])
    # leap year by year
    ymd = [(2020, 2, 29), (2022, 2, 28), (2024, 2, 29), (2026, 2, 28), (2028, 2, 29)]
    assert daterange(Date(2020, 2, 29), Date(2028, 7, 31), "years", stepSize=2) == ListOf(
        Date
    )([Date(y, m, d) for (y, m, d) in ymd])
