...
class Timestamp(Class, Final):
    ...
    def add(self, num: float):
        return Timestamp(ts=self.ts + num)
    ...
    @Entrypoint
    def format(self, utc_offset: int = 0, format: str = "%Y-%m-%dT%H:%M:%S") -> str:
        '''
    ...
    def from_date(year=0, month=0, day=0, hour=0, minute=0, second=0):
        ...
        Returns:
            timestamp (Timestamp): A Timestamp
        '''
        ts = date_to_seconds(year, month, day) + time_to_seconds(hour, minute, second)
        return Timestamp(ts=ts)
